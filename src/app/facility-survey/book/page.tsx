...
const SURVEY_TYPES: { id: string; label: string; Icon: LucideIcon; desc: string }[] = [
  { id: "fire",         label: "Fire Safety Systems Audit",    Icon: Flame,     desc: "Extinguishers, systems, exits" },
  { id: "electrical",   label: "Electrical Assets Audit",      Icon: Zap,       desc: "Panels, wiring, DG sets" },
  { id: "plumbing",     label: "Water Management Systems Audit",Icon: Droplets,  desc: "Water source, pumps, tanks, STP" },
  ...
  return (
    <div>
      <label className="block text-[11px] font-semibold text-[#4a5568] mb-1 tracking-wide uppercase">
        {label}{required && <span className="text-[#15803d] ml-0.5">*</span>}
      </label>
      <input
        type={type} name={name} value={value} placeholder={placeholder} required={required}
        onChange={(e) => onChange(e.target.value)}
        className="cursor-text w-full px-3 py-2 rounded-xl border border-[#d1fae5] bg-white text-[13px] text-[#1a202c] placeholder:text-[#a0aec0] focus:outline-none focus:ring-2 focus:ring-[#15803d]/30 focus:border-[#15803d] transition-all"
      />
    </div>
  )
}
  ...
  return (
    <div>
        ...
        {[0, 1].map(i => {
          ...
          return (
            <div key={i} className={`flex-1 flex items-center gap-2 px-2.5 py-2 rounded-xl border transition-all ${
              slot ? "bg-[#052e16] border-[#052e16]" : "border-dashed border-[#6ee7b7] bg-[#f0fdf4]"
            }`}>
              <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[9px] font-bold ${
                slot ? "bg-white text-[#052e16]" : "border-2 border-[#6ee7b7] text-[#8ba5be]"
              }`}>{i + 1}</div>
              {slot ? (
      ...
      {!pending && slots.length < 2 && (
        <p className="text-[11.5px] font-semibold text-[#15803d] text-center mb-2.5">
          {slots.length === 0
      ...
      <div className="flex items-center justify-between mb-2">
        <button type="button" onClick={prevMonth} className="cursor-pointer p-1 hover:bg-[#f0fdf4] rounded-lg transition-colors">
          <ChevronLeft size={15} className="text-[#4a5568]" />
        </button>
        <span className="text-[13px] font-semibold text-[#052e16]">{MONTH_NAMES[viewMonth]} {viewYear}</span>
        <button type="button" onClick={nextMonth} className="cursor-pointer p-1 hover:bg-[#f0fdf4] rounded-lg transition-colors">
          ...
          return (
            <button
              ...
              className={[
                ...
                sel  ? "bg-[#052e16] text-white font-bold" : "",
                pend ? "bg-[#d1fae5] text-[#166534] font-bold ring-2 ring-[#15803d] ring-offset-1" : "",
                !dis && !sel && !pend && !sun ? "text-[#064e3b] hover:bg-[#f0fdf4]" : "",
                isToday && !sel && !pend ? "ring-2 ring-[#15803d] ring-offset-1 font-bold" : "",
              ].filter(Boolean).join(" ")}
            >{day}</button>
          )
        })}
      ...
      {pending && (
        <div className="mt-3 bg-[#052e16] rounded-xl p-3.5">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {TIME_SLOTS.map(t => (
              <button key={t} type="button" onClick={() => confirmTime(t)}
                className="cursor-pointer px-2.5 py-2 text-[11.5px] font-medium text-white bg-white/10 border border-white/20 rounded-lg hover:bg-white hover:text-[#052e16] transition-all">
                {t}
              </button>
            ))}
          </div>
...
export default function FacilitySurveyPage() {
  ...
  if (submitted) {
    return (
      <>
        <Navigation />
        <main className="min-h-screen bg-[#f0fdf4] flex items-center justify-center px-6 py-16">
          <div className="max-w-sm w-full text-center">
            <div className="w-14 h-14 rounded-full bg-emerald-100 flex items-center justify-center mx-auto mb-4">
              <CheckCircle size={28} className="text-emerald-600" />
            </div>
            <div className="inline-block bg-[#f0fdf4] border border-[#d1fae5] px-5 py-2.5 rounded-xl mb-4">
              <p className="text-[10px] font-semibold text-[#4a5568] uppercase tracking-widest mb-0.5">Booking ID</p>
              <p className="text-[18px] font-bold text-[#052e16] font-mono tracking-widest">{bookingId}</p>
            </div>
            <h1 className="font-serif text-[1.7rem] font-light text-[#052e16] mb-2">Survey Booked!</h1>
            <p className="text-[13px] text-[#718096] font-light leading-relaxed mb-1.5">
              Our team will connect within 24 hours to confirm your
              <strong className="text-[#15803d] font-semibold"> Free AI Facility Health Survey</strong>.
            </p>
            <p className="text-[11.5px] text-[#a0aec0] mb-7">
              Acknowledgement sent to <strong className="text-[#4a5568]">{contact.email}</strong>
            </p>
            <Link href="/"
              className="cursor-pointer inline-flex items-center justify-center gap-2 bg-[#052e16] text-white px-6 py-2.5 rounded-xl text-[13px] font-medium hover:bg-[#064e3b] transition-colors">
  ...
  return (
    <>
      ...
      <main>
        {/* Hero — compact */}
        <section className="bg-[#052e16] py-6 px-6 relative overflow-hidden">
          <div className="absolute inset-0 pointer-events-none opacity-[0.03]"
            style={{ backgroundImage:`radial-gradient(circle at 1px 1px, white 1px, transparent 0)`, backgroundSize:"40px 40px" }} />
          <div className="max-w-lg mx-auto text-center relative">
            <h1 className="font-serif text-[1.55rem] font-light text-white leading-tight mb-1">
              Free AI Facility <span className="italic text-[#6ee7b7]">Health Survey</span>
            </h1>
        ...
        `}</style>
        <div className="bg-[#f0fdf4] border-t border-[#f0fdf4]">
          <div className="max-w-lg mx-auto px-4 sm:px-5 py-5">
                ...
                <div key={num} className="flex items-center flex-1">
                  <div className="flex flex-col items-center gap-0.5 flex-shrink-0">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-colors ${
                      step > num ? "bg-[#052e16] text-white" : step === num ? "bg-[#15803d] text-white" : "bg-[#f0fdf4] text-[#a0aec0]"
                    }`}>
                      {step > num ? <CheckCircle size={11} /> : num}
                    </div>
                    <span className={`text-[9.5px] font-medium hidden sm:block whitespace-nowrap ${
                      step === num ? "text-[#15803d]" : step > num ? "text-[#4a5568]" : "text-[#c0ccd8]"
                    }`}>{label}</span>
                  </div>
                  {i < STEPS.length - 1 && (
                    <div className={`flex-1 h-0.5 mx-1.5 mb-3 sm:mb-0 rounded-full transition-colors ${step > num ? "bg-[#052e16]" : "bg-[#d1fae5]"}`} />
                  )}
                </div>
              ))}
            </div>
            ...
            {/* Card */}
            <div key={step} className={`bg-white rounded-[18px] border border-[#d1fae5] shadow-[0_4px_20px_rgba(5,46,22,0.06)] p-5 ${direction === "forward" ? "step-fwd" : "step-bck"}`}>

              {/* ── Step 1 ── */}
              {step === 1 && (
                <div>
                  <h2 className="text-[14.5px] font-semibold text-[#052e16] mb-0.5">What type of facility?</h2>
                      ...
                      return (
                        <button key={id} type="button" onClick={() => setFacilityType(id)}
                          className={`cursor-pointer flex items-center gap-2.5 px-3 py-2.5 rounded-xl border text-left transition-all ${
                            sel ? "border-[#15803d] bg-[#f0fdf4]" : "border-[#d1fae5] bg-white text-[#4a5568] hover:border-emerald-300 hover:bg-[#f0fdf4]"
                          }`}
                        >
                          <div className={`w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 ${sel?"bg-[#15803d]":"bg-[#f0fdf4]"}`}>
                            <Icon size={12} className={sel?"text-white":"text-[#15803d]"} />
                          </div>
                          <span className={`text-[12.5px] font-medium ${sel?"text-[#052e16]":""}`}>{label}</span>
                          {sel && <CheckCircle size={12} className="text-[#15803d] ml-auto flex-shrink-0" />}
                        </button>
                      )
                    })}
                  </div>
              ...
              {step === 2 && (
                <div>
                  <h2 className="text-[14.5px] font-semibold text-[#052e16] mb-0.5">What would you like surveyed?</h2>
                    ...
                    {SURVEY_TYPES.map(({ id, label, Icon, desc }) => {
                      ...
                      return (
                        <button key={id} type="button" onClick={() => toggleSurvey(id)}
                          className={`cursor-pointer flex items-center gap-2.5 px-3 py-2.5 rounded-xl border text-left transition-all ${
                            sel ? "border-[#15803d] bg-[#f0fdf4]" : "border-[#d1fae5] bg-white hover:border-emerald-300 hover:bg-[#f0fdf4]"
                          }`}
                        >
                          <div className={`w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 ${sel?"bg-[#15803d]":"bg-[#f0fdf4]"}`}>
                            <Icon size={12} className={sel?"text-white":"text-[#15803d]"} />
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-[12px] font-medium leading-tight ${sel?"text-[#052e16]":"text-[#2d3748]"}`}>{label}</p>
                            <p className="text-[10.5px] text-[#4a5568]">{desc}</p>
                          </div>
                          <div className={`w-3.5 h-3.5 rounded border flex-shrink-0 flex items-center justify-center ${sel?"bg-[#15803d] border-[#15803d]":"border-[#cbd5e0]"}`}>
                  ...
                  <button type="button" onClick={() => toggleSurvey("all")}
                    className={`cursor-pointer w-full flex items-center justify-center px-3 py-2.5 rounded-xl border transition-all ${
                      surveyTypes.length === SURVEY_TYPES.length
                        ...
                        : "border-[#d1fae5] bg-white text-[#4a5568] hover:bg-[#f0fdf4]"
                    }`}
                  >
              ...
              {step === 3 && (
                <div className="space-y-5">
                  <div>
                    <h2 className="text-[14.5px] font-semibold text-[#052e16] mb-0.5">Facility details</h2>
                    <p className="text-[11.5px] text-[#718096]">Tell us about the size and layout of your facility.</p>
                  </div>
                  {/* Area */}
                  <div>
                    <label className="block text-[11px] font-semibold text-[#4a5568] mb-1 tracking-wide uppercase">
                      Total Facility Area <span className="text-[#15803d]">*</span>
                    </label>
                    <div className="flex gap-2">
                      <input
                        type="number" value={facilityArea} onChange={e => setFacilityArea(e.target.value)}
                        placeholder="e.g. 25000"
                        className="cursor-text flex-1 px-3 py-2 rounded-xl border border-[#d1fae5] bg-white text-[13px] placeholder:text-[#a0aec0] focus:outline-none focus:ring-2 focus:ring-[#15803d]/30 focus:border-[#15803d] transition-all"
                      />
                      <select
                        value={facilityAreaUnit} onChange={e => setFacilityAreaUnit(e.target.value as "sqft"|"acres")}
                        className="cursor-pointer px-3 py-2 rounded-xl border border-[#d1fae5] bg-white text-[13px] text-[#1a202c] focus:outline-none focus:ring-2 focus:ring-[#15803d]/30 focus:border-[#15803d] transition-all"
                      >
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      ...
                      <button type="button" onClick={addBuilding} disabled={!buildingInput.trim()}
                        className="cursor-pointer flex items-center gap-1 px-3 py-2 bg-[#052e16] hover:bg-[#064e3b] disabled:opacity-40 text-white rounded-xl text-[12px] font-medium transition-colors flex-shrink-0">
                        <Plus size={12} /> Add
                      </button>
                    </div>
                    {buildings.length > 0 ? (
                      <div className="flex flex-wrap gap-1.5">
                        {buildings.map(b => (
                          <div key={b.id} className="cursor-default flex items-center gap-1.5 px-2.5 py-1 bg-[#f0fdf4] border border-[#d1fae5] rounded-lg">
                            <span className="text-[11.5px] font-medium text-[#052e16]">{b.name}</span>
              ...
              {step === 4 && (
                <div>
                  <h2 className="text-[14.5px] font-semibold text-[#052e16] mb-0.5">Preferred survey dates</h2>
                  <p className="text-[11.5px] text-[#4a5568] font-medium mb-3">
                    Select <span className="text-[#052e16] font-bold">2 preferred dates</span> + time windows (Mon–Sat, IST).
              ...
              {step === 5 && (
                <div>
                  <h2 className="text-[14.5px] font-semibold text-[#052e16] mb-0.5">Your contact details</h2>
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="cursor-text w-full px-3 py-2 rounded-xl border border-[#d1fae5] bg-white text-[13px] placeholder:text-[#a0aec0] focus:outline-none focus:ring-2 focus:ring-[#15803d]/30 focus:border-[#15803d] transition-all resize-none"
                      />
                      <button type="button" onClick={handleGps} disabled={gpsLoading}
                        className="cursor-pointer mt-1.5 flex items-center gap-1.5 text-[11.5px] font-medium text-[#15803d] hover:text-[#166534] transition-colors disabled:opacity-50">
                    ...
                    {mapCoords && (
                      <iframe
                        ...
                        className="w-full h-[140px] rounded-xl border border-[#d1fae5]"
            ...
            <div className="flex gap-2.5 mt-3.5">
              {step > 1 && (
                <button type="button" onClick={goBack}
                  className="cursor-pointer flex items-center justify-center gap-1.5 border border-[#d1fae5] text-[#4a5568] px-4 py-2.5 rounded-xl text-[12.5px] font-medium hover:bg-[#f0f4f8] transition-colors">
                  <ArrowLeft size={13} /> Back
                </button>
              )}
              {step < 5 ? (
                <button type="button" onClick={goNext}
                  className="cursor-pointer flex-1 flex items-center justify-center gap-1.5 bg-[#052e16] hover:bg-[#064e3b] text-white py-2.5 rounded-xl text-[13px] font-semibold transition-colors shadow-[0_4px_12px_rgba(5,46,22,0.20)]">
                  Next <ArrowRight size={13} />
                </button>
              ) : (
                <button type="button" onClick={handleSubmit} disabled={submitting}
                  className="cursor-pointer flex-1 flex items-center justify-center gap-1.5 bg-[#052e16] hover:bg-[#064e3b] disabled:opacity-60 text-white py-2.5 rounded-xl text-[13px] font-semibold transition-colors shadow-[0_4px_12px_rgba(5,46,22,0.20)]">
