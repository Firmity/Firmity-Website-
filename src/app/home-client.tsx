...
function HomeBlogSection({ initialPosts }: { initialPosts: LatestPost[] }) {
  // Posts arrive already fetched by the server page so they're in the initial
  // HTML (crawlers that don't run JS see them); the client fetch below is only
  // a fallback for when the server-side read came back empty.
              ...
              return (
                ...
                >
                  ...
                  <div className="relative h-[230px] bg-[#eef3f9] overflow-hidden flex-shrink-0">
                    {post.cover
                      ? <Image src={post.cover} alt={post.title} fill sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" className="object-cover group-hover:scale-[1.04] transition-transform duration-300" />
            ...
            {caseStudies.map(function(post) {
              return (
                ...
                >
                  ...
                  <div className="relative h-[180px] bg-[#eef3f9] overflow-hidden flex-shrink-0">
                    {post.cover
                      ? <Image src={post.cover} alt={post.title} fill sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" className="object-cover group-hover:scale-[1.04] transition-transform duration-300" />
  ...
  return (
    <>
      ...
      <main>
        ...
        <HomeBlogSection initialPosts={initialPosts} />
